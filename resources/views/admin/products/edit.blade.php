...
<div style="max-width: 900px; margin: 0 auto;">
    <div style="margin-bottom: 1rem; display: flex; justify-content: space-between; align-items: center;">
        ...
        <a href="{{ route('products.show', $product->slug ?: $product->id) }}" target="_blank" class="admin-btn admin-btn-outline admin-btn-sm">
            View Live Product ↗
        </a>
    </div>
    ...
    <div class="admin-card">
        <div class="admin-card-header">
            <div>
                <h2 class="admin-card-title">Edit Product: {{ $product->name }}</h2>
                <p style="font-size:0.8rem;color:var(--admin-text-muted);margin:0.25rem 0 0;">Update technical details, category classification, images, and specifications.</p>
            </div>
        </div>
        ...
        <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-grid-2">
                <div class="form-group">
                    ...
                    <input type="text" id="title" name="title" value="{{ old('title', $product->name ?: $product->title) }}" required class="form-control">
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="category_id" name="category_id" required class="form-control" style="font-family: monospace; font-size: 0.86rem;" onchange="updateProductCategoryPath()">
                        @foreach($selectTree as $item)
                            <option value="{{ $item['id'] }}" data-path="{{ $item['breadcrumb'] }}" data-level="{{ $item['level'] }}" {{ old('category_id', $product->category_id) == $item['id'] ? 'selected' : '' }}>
                    ...
                    <div id="catBreadcrumbPreview" style="margin-top: 0.35rem; font-size: 0.76rem; color: #475569; font-weight: 500;">
                        Hierarchy: <span id="catPathText" style="color: #0F172A; font-weight: 700;">{{ $product->category_breadcrumb }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="form-grid-2">
                <div class="form-group">
                    ...
                    <input type="text" id="slug" name="slug" value="{{ old('slug', $product->slug) }}" class="form-control">
                    <span style="font-size: 0.72rem; color: #94A3B8;">Public URL: /products/{{ $product->slug }}</span>
                </div>
                ...
                <div class="form-group">
                    ...
                    <div style="display: flex; gap: 0.75rem; align-items: center; margin-bottom: 0.5rem;">
                        ...
                        @endphp
                        <img src="{{ asset($imgSrc) }}" alt="{{ $product->name }}" style="width: 54px; height: 44px; object-fit: cover; border-radius: 6px; border: 1px solid #CBD5E1;" onerror="this.src='{{ asset('images/hero-engineering.png') }}'">
                    ...
                    <div>
                        ...
                        <input type="text" name="image_url" value="{{ old('image_url', $product->image) }}" class="form-control" style="font-size: 0.78rem; padding: 0.35rem 0.6rem; margin-top: 0.2rem;">
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="short_description" name="short_description" value="{{ old('short_description', $product->short_description) }}" class="form-control">
            </div>
                    ...
                    @if(count($specs) > 0)
                        @foreach($specs as $spec)
                            <div class="spec-row" style="display: flex; gap: 0.75rem; align-items: center;">
                                <input type="text" name="spec_keys[]" value="{{ is_array($spec) ? ($spec[0] ?? '') : '' }}" placeholder="Property / Spec Name" class="form-control" style="flex: 1;">
                                <input type="text" name="spec_values[]" value="{{ is_array($spec) ? ($spec[1] ?? '') : '' }}" placeholder="Value" class="form-control" style="flex: 1.5;">
                                <button type="button" class="remove-spec-btn admin-btn admin-btn-danger admin-btn-sm" style="padding: 0.5rem 0.65rem;">✕</button>
                            </div>
                        @endforeach
                    @else
                        <div class="spec-row" style="display: flex; gap: 0.75rem; align-items: center;">
                            <input type="text" name="spec_keys[]" value="Material" placeholder="Property / Spec Name" class="form-control" style="flex: 1;">
                            <input type="text" name="spec_values[]" value="High Precision Alloy" placeholder="Value" class="form-control" style="flex: 1.5;">
                            <button type="button" class="remove-spec-btn admin-btn admin-btn-danger admin-btn-sm" style="padding: 0.5rem 0.65rem;">✕</button>
                        </div>
                    @endif
                </div>
