...
@section('page-title', 'Add Category to Tree')
...
<div style="max-width: 800px; margin: 0 auto;">
    ...
    <div class="admin-card">
        <div class="admin-card-header">
            <div>
                <h2 class="admin-card-title">Create New Category / Subcategory</h2>
                <p style="font-size:0.8rem;color:var(--admin-text-muted);margin:0.25rem 0 0;">
                    Assign this category to a parent to create Level 2, Level 3, or Level 4 subcategories.
                </p>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('admin.categories.store') }}">
            ...
            <div class="form-group" style="background: #F8FAFC; border: 1px solid #E2E8F0; border-radius: 8px; padding: 1.25rem; margin-bottom: 1.5rem;">
                ...
                <p style="font-size: 0.78rem; color: #64748B; margin: 0 0 0.75rem;">
                    Select <strong>"None (Top-Level Root - Level 1)"</strong> to create a main category, or select an existing parent to nest it as Level 2, Level 3, or Level 4.
                </p>
                <select name="parent_id" id="parent_id" class="form-control" style="font-family: monospace; font-size: 0.88rem; padding: 0.65rem 0.85rem;" onchange="updateLevelPreview()">
                    <option value="" data-level="1" {{ empty($preselectedParentId) ? 'selected' : '' }}>
                        [ROOT LEVEL 1] None (Top-Level Category)
                    </option>
                    @foreach($selectTree as $item)
                        <option value="{{ $item['id'] }}" data-level="{{ min(4, $item['level'] + 1) }}" {{ $preselectedParentId == $item['id'] ? 'selected' : '' }}>
                ...
                <div id="levelPreviewBox" style="margin-top: 0.75rem; font-size: 0.8rem; padding: 0.5rem 0.75rem; border-radius: 6px; background: #EEF2FF; color: #3730A3; font-weight: 600; display: inline-flex; align-items: center; gap: 0.4rem;">
                    <span id="levelPreviewText">Resulting Depth: Level 1 (Top-Level Root)</span>
                </div>
            </div>
            ...
            <div class="form-grid-2">
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name') }}" required class="form-control" placeholder="e.g. Reamers & Deburring, Hand Reamers" autofocus oninput="generateSlug(this.value)">
                </div>

                <div class="form-group">
                    <label class="form-label" for="slug">URL Slug (Auto-generated)</label>
                    <input type="text" id="slug" name="slug" value="{{ old('slug') }}" class="form-control" placeholder="e.g. reamers-deburring" style="font-family: monospace;">
                </div>
            </div>
            ...
            <div class="form-grid-2">
                <div class="form-group">
                    ...
                    <select name="color" id="color" class="form-control">
                        @foreach($colors as $val => $lbl)
                            <option value="{{ $val }}" {{ old('color') == $val ? 'selected' : '' }}>{{ $lbl }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', 0) }}" class="form-control" placeholder="0">
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea id="description" name="description" rows="3" class="form-control" placeholder="Brief overview of tools and precision products in this category...">{{ old('description') }}</textarea>
            </div>

            <!-- Is Active -->
            <div class="form-group" style="margin-top: 1rem;">
                <label style="display: flex; align-items: center; gap: 0.6rem; cursor: pointer;">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} style="width: 16px; height: 16px; accent-color: var(--admin-accent);">
                    <span style="font-size: 0.85rem; font-weight: 600; color: #1E293B;">Active &amp; Visible in Mega-Menu</span>
                </label>
            </div>

            <div style="margin-top: 2rem; display: flex; gap: 1rem; align-items: center; border-top: 1px solid #E2E8F0; padding-top: 1.25rem;">
                <button type="submit" class="admin-btn admin-btn-accent" style="padding: 0.65rem 1.4rem;">
                    <span>Save Category to Tree</span>
                </button>
